<script lang="ts">
	...
	function submit(e: MouseEvent) {
		...
		toaster.success({
			title: 'All set!',
			description: `${form.workspaceName} is ready to go.`
		});
		submitted = true;
	}
	...
	{#if submitted}
		<AnimateIn>
			<div class="flex flex-col items-center gap-3 card p-10 text-center">
				<div
					class="flex size-14 items-center justify-center rounded-full bg-success-500/10 text-success-500"
				>
					<CheckCircle2Icon class="size-7" />
				</div>
				<h2 class="h3">You’re all set!</h2>
				<p class="max-w-sm text-sm opacity-70">
					{form.workspaceName} is ready to go. Invite your team and start building.
				</p>
				<button class="mt-2 btn press preset-tonal btn-sm" onclick={reset}>Start over</button>
			</div>
		</AnimateIn>
	{:else}
		<Steps count={steps.length} step={currentStep} onStepChange={(e) => (currentStep = e.step)}>
			<Steps.List>
				{#each steps as item, index (index)}
					<Steps.Item {index}>
						...
						<!-- Separator must stay inside Steps.Item — it consumes the item context (v5) -->
			...
			<Steps.Content index={3}>
				<div class="mt-6 space-y-4 card p-6">
					...
					<button class="btn w-full press preset-filled-primary-500" onclick={submit}>Submit</button
					>
				</div>
			</Steps.Content>
