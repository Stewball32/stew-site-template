...
| `seed/`            | `Run()`         | Dev-only data seeder — compiled with `-tags dev`, no-op in production |
...
### Resolver (no registry)

Resolvers have no `init()` or `RegisterAll()` — they're just exported functions. Each file exports one function taking `app core.App` as the first parameter and returning data from PocketBase. They are how the `Service` wrapper in `service.go` satisfies cross-system interfaces (e.g. `pbiface.Users`).

1. Create a new file in `resolvers/` named after the lookup (e.g., `find_user_by_discord_id.go`)
2. Export a single function (e.g., `FindUserByDiscordID(app, discordID)`)
3. Call it from any trigger: routes, hooks, or — via the `Service` wrapper — from Discord commands and WebSocket handlers

Resolvers are the shared lookup layer. When multiple triggers all need the same PB query, extract it here.
